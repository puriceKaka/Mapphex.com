  ...
  const redirectActiveOrganization = async () => {
    ...
    try {
      ...
      if (settings.agreementAccepted !== true) {
        location.replace(`organization/organization-agreement.html?tenant=${tenant}`);
      } else if (Array.isArray(settings.installedPortals) && settings.installedPortals.length) {
        location.replace(`organization/organization-workspace.html?tenant=${tenant}`);
      } else {
        location.replace(`organization/portal-selection.html?tenant=${tenant}`);
      }
    } catch {
